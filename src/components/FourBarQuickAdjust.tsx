...
function FourBarField({ corner, data, spec, barLabel, compact, onFieldChange }: {
  key?: string;
  corner: 'lr' | 'rr';
  data: CornerSetup;
  spec: FieldSpec;
...
function BarSection({ corner, data, bar, compact, onFieldChange }: {
  key?: string;
  corner: 'lr' | 'rr';
  data: CornerSetup;
  bar: BarSpec;
...
export default function FourBarQuickAdjust({ setup, onFieldChange, compact = false, disabledReason, onHelp }: FourBarQuickAdjustProps) {
  if (!setup) {
    return <div className="rounded-xl border border-dashed border-outline-variant bg-surface-container p-3 font-mono text-sm text-on-surface-variant">{disabledReason ?? 'Select a setup for this car to adjust four-bar.'}</div>;
  }
  return (
    <div className={compact ? 'space-y-3' : 'space-y-3 rounded-xl border-2 border-primary/60 bg-surface-container-high p-3'}>
      <div className={`flex items-start gap-2 ${compact ? 'justify-end' : 'justify-between'}`}>
