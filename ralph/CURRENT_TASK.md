...
**Status:** READY — exactly one Terra High initial build; stop for independent SOL QA
...
7. `git diff --check`; exact four-file boundary including the authorized deletion; protected paths clean; worktree clean after commit.
...
No B4, Chunk B QA advancement, Chunk C, C1–C5, or D1–D3. No `src/types.ts`, NumberStepper, SetupView, RaceWeekendView, CSS, other component or harness, schema, RLS, migrations, Supabase configuration, native, Android, release, package/config, credentials, branch refs, push, deploy, merge, production, or `master` changes. Do not alter dual-write data, delete/undo/cascade/zero-row behavior, retry-queue semantics, pull merge/throttle/lifecycle behavior, or B1/B2 interaction semantics.
