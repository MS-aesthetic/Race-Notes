...
**Decision.** As v1. Item 6 dissolves most of the deadlock. Residuals: genuinely locked setups stay individually undeletable (protected, correct) but the disabled Delete states *why* from the reason enum; and the cleanup path becomes a **confirmed cascade delete on the car** — Garage enumerates exactly what blocks ("3 setups, 1 tire set, 2 shock records"), strong confirmation, then deletes each scoped record through the existing protected pairing per table before the car. No new delete primitives, no queue bypass, no orphans, no resurrection by construction. Mid-cascade failure surfaces as an explicit error (item 10), never Saved; queued intents retry under existing rules. Note for item 7: embedded session snapshots survive the cascade untouched — historical sessions remain self-describing after their setup is gone, which is a new argument *for* the embedded-copy decision. **Depends on items 6, 10, and 11a landing first.**

**Files.** `App.tsx:443-481` (orchestration), `GarageView.tsx:185-195` (enumeration + confirm), reusing `App.tsx:1445-1448`/`1847-1871`.
**Harness.** Cascade-pairing assertion (every cascaded record hits both queue and push). **ISOLATED.** **LOE: M-L (8-12h).**
...
**Task D3 — Car cascade delete (item 12). ISOLATED.** Files: `App.tsx:443-481` orchestration, `GarageView.tsx:185-195` enumeration/confirm, reusing `App.tsx:1445-1448`/`1847-1871`. Change: enumerate blocking records with reasons; strong confirmation; cascade via existing queue+push pairing per table; active-car reassignment preserved; disabled setup-Delete buttons state their reason and link here. **No new delete primitives; no queue bypass; no pull-filtering changes.** Harness: cascade-pairing assertion. Acceptance: car deletable after confirmation without manually reaching zero setups; no orphans, no resurrection; session snapshots survive; mid-cascade failure shows an explicit error, never Saved.
...
A1–A4 + Chunk A QA: PASS. B1–B3 + Chunk B QA: PASS. C1: PASS. C2: PASS 98/100 on QA attempt 2 (`253897a` product + `13b556f` EOL-portability repair). C2.5: PASS 100/100 on QA attempt 2 (`8ab870a` feature + `2235c7d` bounded geometry repair). C3: PASS 100/100 on QA attempt 1 (`e8d7016`). C4: PASS 100/100 on QA attempt 1 (`83230c9`). C5: PASS 100/100 on QA attempt 1 (`1a3d492`). Chunk C integrated QA: PASS 100/100 on attempt 1. D1: PASS 100/100 on attempt 1 (`1ca3576`). D1's exact two-file isolated change requires Supabase-selected matching-id proof before a shared delete succeeds; the unchanged replay retains zero-row/error intents, retries at exactly 5 seconds, filters queued rows from pulls, and preserves status/auth-generation priority. Its 62-assertion/13-mutation production-bound proof, 11 focused regressions, exact 23/24 matrix, three-error lint baseline, 566-module build, correct-site three-phone draft shell, Java 21 debug APK, protected/clean checks, and cavecrew review all pass. No D1 repair remains. The owner-requested pre-D2 setup-label microfix also passed 100/100 at `dc5c63d`. D2 attempt 1 candidate `803d5d0` scored 92/100 and failed only its live result-copy gate: device-only correctly cleared local data and cloud fixtures returned on reload, but the toast falsely said the action could not be completed because the new success string fell through `componentInfoNotice()` to `operation-failed`. Data behavior, dialog, automated gates, build, scope, and reviewer checks pass. A two-file SOL High structured-copy repair is active in `ralph/CURRENT_TASK.md`; everywhere live deletion, D3, and later work remain blocked. Remaining: D2 repair/pass, D3, Chunk D QA, Chunk E (E1–E3), final full-sprint QA, Part 6.4 handoff document.

The owner supplied a dedicated Crew Chief account for D2 destructive QA and authorized clearing only that account's owned racing data. Credentials remain outside repository files and worker contracts. The SOL High builder uses mocks only. Primary QA runs device-only first to prove honest re-download, runs everywhere last, and leaves owned racing data cleared. Account deletion, authentication deletion, team or membership changes, and deletion of another user's data remain forbidden.
