...
| `C:\Users\maxx\antigravity\Race-Notes\.worktrees\v3` | `preview-v3` | **Active UX dev tree.** Chunks 1–9 and UXF-1/2/3/4/5/6/8 complete. UXF-7 proposal is drafted at `docs/MAIN_CHECKLIST_REDESIGN_PROPOSAL.md` and awaits Maxx approval/redline; no checklist application code changed. Recommended target: active-first list, Manage sheet, one-tap completion/Undo, preserved History, explicit clear/reset/maintenance rules, idempotent saved lists, compact Dashboard, and one optional JSON field with no SQL. UXF-10 cannot start before approval. UXF-6 migration `20260714215528` remains applied/verified. No production/push/merge/package/native/APK change. Local gitignored debug-only Gradle scaffolding may exist from prior QA; it is not committed release configuration. |
